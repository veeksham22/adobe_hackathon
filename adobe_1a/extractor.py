...
def extract_outline_from_pdf(pdf_path):
    ...
    for page_num, page in enumerate(doc, start=1):
        ...
        for block in blocks:
            ...
            # Group consecutive lines that might form a single heading
            heading_lines = []
            current_heading_level = None
            current_heading_text = ""
            
            for line in block["lines"]:
                # Check if this line contains heading-level text
                ...
                for span in line["spans"]:
                    ...
                    if level in ["H1", "H2", "H3"]:
                        ...
                        # Only include text if it matches the dominant level for this line
                        if level == line_heading_level:
                            line_text += text + " "
                
                line_text = line_text.strip()
                
                # If this line has heading text
                if line_heading_level and line_text:
                    # If it's the same level as previous line and close spatially, combine
                    if (current_heading_level == line_heading_level and 
                        current_heading_text and
                        not re.match(r'^\d+\.', line_text)):  # Don't combine if new line starts with number
                        current_heading_text += " " + line_text
                    else:
                        # Save previous heading if exists
                        if current_heading_level and current_heading_text:
                            outline.append({
                                "level": current_heading_level,
                                "text": re.sub(r'\s+', ' ', current_heading_text).strip(),
                                "page": page_num
                            })
                    ...
                    # Non-heading line breaks the sequence
                    if current_heading_level and current_heading_text:
                        outline.append({
                            "level": current_heading_level,
                            "text": re.sub(r'\s+', ' ', current_heading_text).strip(),
            ...
            # Don't forget the last heading in the block
            if current_heading_level and current_heading_text:
                outline.append({
                    "level": current_heading_level,
                    "text": re.sub(r'\s+', ' ', current_heading_text).strip(),
